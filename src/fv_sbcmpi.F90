MODULE fv_sbcmpi
   ...
   !!          1.1 ! 08/2020 I. Kuznetsov, MPI version
   ...
   public  sbcmpi_ini  ! routine called before 1st time step (open files, read namelist,...)
   public  sbcmpi_do   ! routine called each time step to provide a sbc fileds (wind,...)
...
!   real(wp), allocatable, save, dimension(:), public, target     :: qns   ! downward non solar heat over the ocean [W/m2]
!   real(wp), allocatable, save, dimension(:), public, target     :: qsr   ! downward solar heat over the ocean [W/m2] surface_downwelling_photosynthetic_radiative_flux
!   real(wp), allocatable, save, dimension(:), public, target     :: emp   ! evaporation minus precipitation        [kg/m2/s]
...
!   character(len=256) :: tmp_str
  ...
  ! length of arrays in INfiles
   integer(kind=MPI_OFFSET_KIND) :: nc_Nlon
   integer(kind=MPI_OFFSET_KIND) :: nc_Nlat
   integer(kind=MPI_OFFSET_KIND) :: nc_Ntime
  ...
  ! flip latitude from infiles (for example  NCEP-DOE Reanalysis 2 standard)
  integer, save              :: flip_lat ! 1 if we need to flip
!============== NETCDF ==========================================

  real(wp), allocatable, dimension(:,:,:)  :: sbcdata1,sbcdata2
  integer :: sbcdata_iold
...
!      integer              :: nf_dims(4) ! dimensions (temporal)
      ...
      !open file
      if (mype==0) then
         iost = nfmpi_open(MPI_COMM_SELF, flf%file_name, NF_NOWRITE, &
                         MPI_INFO_NULL, ncid)
         call check_nferr(iost,flf%file_name)

         ! get dimensions
         iost = nfmpi_inq_dimid(ncid, "latitude", id_latd)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_dimid(ncid, "longitude", id_lond)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_dimid(ncid, "time", id_timed)
         call check_nferr(iost,flf%file_name)

         ! get variable id
         iost = nfmpi_inq_varid(ncid, "longitude", id_lon)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_varid(ncid, "latitude", id_lat)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_varid(ncid, "time", id_time)
         call check_nferr(iost,flf%file_name)

!         !  get dimensions size
         iost = nfmpi_inq_dimlen(ncid, id_latd, nc_Nlat)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_dimlen(ncid, id_lond, nc_Nlon)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_inq_dimlen(ncid, id_timed, nc_Ntime)
         call check_nferr(iost,flf%file_name)
      ...
      ALLOCATE( sbcdata1(i_totfl,nc_Nlon,nc_Nlat), sbcdata2(i_totfl,nc_Nlon,nc_Nlat),&
                &      STAT=sbc_alloc )
      if( sbc_alloc /= 0 )   STOP 'sbcmpi_ini: failed to allocate arrays'
      sbcdata1 = 0.0_wp
      sbcdata2 = 0.0_wp
      ...
      if (mype==0) then
         ...
         nf_start(1)=1
         nf_edges(1)=nc_Nlat
         iost = nfmpi_get_vara_double_all(ncid, id_lat, nf_start, nf_edges, nc_lat)
         call check_nferr(iost,flf%file_name)
         nf_start(1)=1
         nf_edges(1)=nc_Nlon
         iost = nfmpi_get_vara_double_all(ncid, id_lon, nf_start, nf_edges, nc_lon)
         call check_nferr(iost,flf%file_name)
         nf_start(1)=1
         nf_edges(1)=nc_Ntime
         iost = nfmpi_get_vara_double_all(ncid, id_time, nf_start, nf_edges, nc_time)
         call check_nferr(iost,flf%file_name)
         iost = nfmpi_close(ncid)
         call check_nferr(iost,flf%file_name)
   ...
   SUBROUTINE nc_sbc_ini_fillnames(yyear)
      ...
      !filenameend = ADJUSTL(trim(filenameend))
      !filenameend='       '
      filenameend = ADJUSTL(trim(filenameend))
...
!Here is no need to do exchange, while we calculate for all nodes, probably
...
!      integer,intent(in)   :: isec ! initialization seconds
      ...
      integer              :: id_data
      integer(kind=MPI_OFFSET_KIND) :: nf_start(4)
      integer(kind=MPI_OFFSET_KIND) :: nf_start_p1(4)
      integer(kind=MPI_OFFSET_KIND) :: nf_edges(4)
!      integer              :: zero_year,yyyy,mm,dd
!      character(len = 256) :: att_string ! attribute
...
!      real(wp), allocatable, dimension(:,:,:)  :: sbcdata1,sbcdata2
      ...
      integer              :: requests(2), statuses(2), totrequests
      integer              :: sbc1_new,sbc2_new
      real(wp)             :: tmpdata1(nc_Nlon,nc_Nlat),tmpdata2(nc_Nlon,nc_Nlat)
...
!      if( sbc_alloc /= 0 )   STOP 'getcoeffld: failed to allocate arrays'
     ...
     !read data from file
      ...
      nf_start_p1(1)=1
      nf_start_p1(2)=1
      nf_start_p1(3)=t_indx_p1


      if (sbcdata_iold==1) then
      ! copy data from previews file step
            sbcdata1 = sbcdata2
      endif
      if (t_indx_p1<=t_indx) then
      ! copy data from previews file step
      ! potential problem with switch between files
            sbcdata2 = sbcdata1
      end if
      sbc1_new = 0
      sbc2_new = 0
      do fld_idx = 1, i_totfl
         totrequests = 0
         !open file
         iost = nf90mpi_open(MPI_COMM_WORLD, sbc_flfi(fld_idx)%file_name, NF_NOWRITE, &
                         MPI_INFO_NULL, ncid)
         call check_nferr(iost,sbc_flfi(fld_idx)%file_name)
         ! get variable id
         iost = nf90mpi_inq_varid(ncid, sbc_flfi(fld_idx)%var_name, id_data)
         call check_nferr(iost,sbc_flfi(fld_idx)%file_name)

         if (sbcdata_iold==0) then
            ! read new data from file
            totrequests = totrequests+1
            iost = nfmpi_iget_vara_double(ncid, id_data, nf_start, nf_edges, &
                           & tmpdata1, requests(totrequests))
!            iost = nfmpi_get_vara_double_all(ncid, id_data, nf_start, nf_edges, &
!                           & sbcdata1(fld_idx,:,:))
         ...
         if (t_indx_p1>t_indx) then
             ...
             totrequests = totrequests + 1
             iost = nfmpi_iget_vara_double(ncid, id_data, nf_start_p1, nf_edges, &
                              & tmpdata2, requests(totrequests))
!             iost = nfmpi_get_vara_double_all(ncid, id_data, nf_start_p1, nf_edges, &
!                              & sbcdata2(fld_idx,:,:))
             call check_nferr(iost,sbc_flfi(fld_idx)%file_name)
             sbc2_new = 1
         endif
         if (totrequests>0) then
            iost = nf90mpi_wait_all(ncid, totrequests, requests(1:totrequests), statuses(1:totrequests))
            call check_nferr(iost,sbc_flfi(fld_idx)%file_name)
         endif
         if (sbcdata_iold==0) then
            sbcdata1(fld_idx,:,:) = tmpdata1
         endif
         if (t_indx_p1>t_indx) then
            sbcdata2(fld_idx,:,:) = tmpdata2
         endif
         iost = nf90mpi_close(ncid)
         call check_nferr(iost,sbc_flfi(fld_idx)%file_name)
      end do !fld_idx

      sbcdata_iold=1  !next time data will be copied to sbcdata1
      ...
      do fld_idx = 1, i_totfl
         ...
         ! bi-linear space interpolation, and time interpolation ,
         ...
         do ii = 1, node_size
            ...
            ! if point inside forcing domain
               denom = (x2 - x1)*(y2 - y1)
               data1 = ( sbcdata1(fld_idx,i,j)   * (x2-x)*(y2-y)   + sbcdata1(fld_idx,ip1,j)    * (x-x1)*(y2-y) + &
                     sbcdata1(fld_idx,i,jp1) * (x2-x)*(y-y1)   + sbcdata1(fld_idx,ip1, jp1) * (x-x1)*(y-y1)     ) / denom
               data2 = ( sbcdata2(fld_idx,i,j)   * (x2-x)*(y2-y)   + sbcdata2(fld_idx,ip1,j)    * (x-x1)*(y2-y) + &
                     sbcdata2(fld_idx,i,jp1) * (x2-x)*(y-y1)   + sbcdata2(fld_idx,ip1, jp1) * (x-x1)*(y-y1)     ) / denom
            else if ( extrp == 1 ) then !  "extrapolation" in x direction
               denom = (y2 - y1)
               data1 = ( sbcdata1(fld_idx,i,j)   * (y2-y)   + sbcdata1(fld_idx,ip1, jp1) * (y-y1) ) / denom
               data2 = ( sbcdata2(fld_idx,i,j)   * (y2-y)   + sbcdata2(fld_idx,ip1, jp1) * (y-y1) ) / denom
            else if ( extrp == 2 ) then !  "extrapolation" in y direction
               denom = (x2 - x1)
               data1 = ( sbcdata1(fld_idx,i,j)   * (x2-x)   + sbcdata1(fld_idx,ip1, jp1) * (x-x1) ) / denom
               data2 = ( sbcdata2(fld_idx,i,j)   * (x2-x)   + sbcdata2(fld_idx,ip1, jp1) * (x-x1) ) / denom
            else if ( extrp == 3 ) then !  "extrapolation" in x and y direction
               data1 = sbcdata1(fld_idx,i,j)
               data2 = sbcdata2(fld_idx,i,j)
            end if
...
!      now_date = ndate+nsec/86400.0_wp
   ...
   SUBROUTINE sbcmpi_ini
      ...
      if (mype==0) print *, "Start: Ocean forcing initialization."
      ...
      if( iost == 0 ) then
         ...
         !print *, mype,'     file   : ', 'namelist_bc.nml',' open ok'
      else
...
! ========================== NC PART start===============================================================
      ...
      if (mype==0) print *, "DONE:  Ocean forcing inizialization."
   END SUBROUTINE sbcmpi_ini

   SUBROUTINE sbcmpi_do
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE sbcmpi_do ***
      !!
...
!                  &  qns, emp, qsr)
   ...
   SUBROUTINE binarysearch(length, array, value, ind)!, delta)
      ...
      IMPLICIT NONE
      integer(kind=MPI_OFFSET_KIND),  intent(in) :: length
