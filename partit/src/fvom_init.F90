...
!!$  call par_init
  ...
  interm_t = finish_t
  call test_tri_ini ! checks quads as well
  call find_edges_ini
  call find_elem_neighbors_ini
  call find_levels !SH probably wrong, check quads
...
use g_parsup
implicit none
INTEGER :: nodes(4), elems(4), eledges(4)
...
USE g_PARSUP
implicit none
integer    :: elem, eledges(4), elem1, j, n, elnodes(4)
allocate(elem_neighbors(4,elem2D))
elem_neighbors=0
DO elem=1,elem2D
   
   eledges=elem_edges(1:4,elem)
   DO j=1,4
...
END DO    
 ...
 do n=1,elem2D
    elnodes=elem2D_nodes(:,n)
    if (elnodes(4)==elnodes(1)) then !triangle
       nod_in_elem2D_num(elnodes(1:3))=nod_in_elem2D_num(elnodes(1:3))+1
    else ! quad
 ...
 do n=1,elem2D   
    ...
    if (elnodes(4)/=elnodes(1)) nod_in_elem2D(nod_in_elem2D_num(elnodes(4)),elnodes(4))=n
 end do  
